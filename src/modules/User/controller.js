...
const userController = {
    ...
    login: async (req, res, next) => {
        try {
            ...
            if (!user)
                throw new ApiError(400, 'unable to find user with the spécified pair email/password');

            user.access_token = jwt.sign({ id: user.id, email: user.email }, config.jwt_secret, { expiresIn: '5m' });
            user.refresh_token = jwt.sign({ id: user.id }, config.jwt_secret, { expiresIn: '60d' });
